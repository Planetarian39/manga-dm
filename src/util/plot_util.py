...
def _posterior_histogram_bin_edges(samples: np.ndarray) -> np.ndarray:
    ...
    for bin_strategy in ("auto", "sturges"):
        ...
        if bin_edges.size >= 2 and np.all(np.isfinite(bin_edges)) and np.all(np.diff(bin_edges) > 0):
            return bin_edges
...
def plot_posterior_1d_hdi(
    ...
    hdi_probs : tuple[float, ...]
        Equal-tailed interval probabilities to compute and display. Values are sorted from narrow to wide.
    point_estimate : str
        Currently supports "median" or "mean".
    annotate : bool
        Whether to display the numeric summary box.
    show_legend : bool
        Whether to display the legend.
    show_interval_bars : bool
        Whether to draw ETI interval bars near the baseline.
    fill_alphas : tuple[float, ...]
        Fill alpha values for ETI regions, ordered from narrow to wide.
    linewidth : float
    ...
    if show_interval_bars:
        ...
        min_y_frac = 0.08
        max_y_frac = 0.16
        y_fracs = np.linspace(max_y_frac, min_y_frac, n_intervals)
        ...
        for y_frac, (_, (low, high)) in zip(y_fracs, intervals):
            ...
            ax.vlines([low, high], y_bar - tick_height, y_bar + tick_height, color=base_color, linewidth=1.4, alpha=0.9)

    if annotate:
        # text_lines = [f"{point_label} = {formatter(point_value)}"]
        text_lines = []
...
class PlotUtil:
    ...
    def plot_rv_curves(self, rv_data_list, plateifu: str="", title: str="", savedir: str=None, savefilename: str=None):
        ...
        for rv_data in rv_data_list:
            ...
            label = rv_data.get('label', f'Velocity')
            ...
            residuals = rv_data.get('res_map', None)
            ...
            if linestyle:
                ...
                ax.plot(r_valid, V_valid, alpha=alpha, label=label, color=color, linestyle=linestyle, zorder=zorder)
            else:
                ax.scatter(r_valid, V_valid, alpha=alpha, label=label, color=color, marker='o', s=size, zorder=zorder)

        # ax.set_title(f"{plateifu} {title}")
        ...
        if savedir is not None:
            ...
            fig.savefig(f"{savedir}/{filename}.png", bbox_inches='tight')
            fig.savefig(f"{savedir}/{filename}.pdf", format='pdf', bbox_inches='tight')
            plt.close(fig)
        else:
            plt.show()
            plt.close(fig)
