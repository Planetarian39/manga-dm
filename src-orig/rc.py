...
GSIGMA_MAX = config.get("thresholds", {}).get("GSIGMA_MAX", 0.0)
USE_GSIGMA_INST_CORR = config.get("thresholds", {}).get("USE_GSIGMA_INST_CORR", True)
...
RMAX_RT_FACTOR = config.get("thresholds", {}).get("RMAX_RT_FACTOR", 2)
...
######################################################
def test_process(PLATE_IFU: str, mode: str="mcmc", n_samples: int=0, check: bool=True) -> None:
    ...
    vel_rot.set_fit_model(mode)
    vel_rot.set_n_samples(n_samples)
    ...
    V_rot_hdi_low = fit_result.get('vel_rot_hdi_low')
    V_rot_hdi_high = fit_result.get('vel_rot_hdi_high')
    V_rot_samples = fit_result.get('vel_rot_samples')
    ...
    if check:
        if not quality_gate['passed']:
            ...
            print(f"Fitting results failure for {PLATE_IFU}, {quality_gate['summary']}, reasons: {fail_reason_text}, NRMSE(diag): {NRMSE:.3f}, CHI_SQ_V(diag): {CHI_SQ_V:.3f}, skipping...")
            return
        else:
            print(f"Fitting results passed for {PLATE_IFU}, {quality_gate['summary']}, NRMSE(diag): {NRMSE:.3f}, CHI_SQ_V(diag): {CHI_SQ_V:.3f}")

    r_disp_map, V_disp_map, ivar_obs_map = vel_rot.get_vel_obs_disp(inc_rad_fit, V_sys_fit, phi_delta_fit)

    rv_plot_data = []
    show_hdi = not (n_samples > 0)

    if n_samples > 0 and V_rot_samples is not None:
        V_rot_samples = np.asarray(V_rot_samples, dtype=float)
        if V_rot_samples.ndim == 2 and V_rot_samples.shape[1] > 0:
            for idx in range(V_rot_samples.shape[1]):
                rv_plot_data.append({
                    'r_map': r_rot_fit,
                    'V_map': V_rot_samples[:, idx],
    ...
    rv_plot_data.extend([
        {'r_map': r_disp_map, 'V_map': V_disp_map, 'title': "Observe", 'color': COLOR_DATA_POINTS, 'label': 'Observe Velocity'},
        {
            'r_map': r_rot_fit,
            'V_map': V_rot_fit,
            ...
            'alpha': 0.95,
            'fill_alpha': 0.16,
            'fill_label': f'Posterior {HDI_PROB2:.0%} HDI' if show_hdi else None,
            'label': 'Median',
        },
    ])

    plot_util.plot_rv_curves(rv_plot_data, plateifu=PLATE_IFU)
...
def main(ifu: str=None, mode: str="mcmc", n_samples: int=0):
    plate_ifu_list = []
    check = True
    except_info = True
    ...
    for plate_ifu in plate_ifu_list:
        ...
        if except_info:
            test_process(plate_ifu, mode=mode, n_samples=n_samples, check=check)
        else:
            try:
                test_process(plate_ifu, mode=mode, n_samples=n_samples, check=check)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", type=str, choices=["fit", "mcmc"], default="mcmc", help="Fitting mode: 'fit' for least-squares, 'mcmc' for Bayesian inference")
    parser.add_argument("--n-samples", type=int, default=0, help="Number of posterior sample curves to overlay in the R-V plot; 0 disables sample curve extraction and plotting")
    args = parser.parse_args()

    main(ifu=args.ifu, mode=args.mode, n_samples=args.n_samples)
