...
from math import inf, log, log10
import os
...
from util.arviz_compat import ensure_arviz_compat, get_arviz_api, get_summary_interval_columns, summary_with_compat
...
INFER_ESS_THRESHOLD = config.get("thresholds", {}).get("INFER_ESS_THRESHOLD", 200)
...
PPC_MEAS_SIGMA_SCALE = config.get("thresholds", {}).get("PPC_MEAS_SIGMA_SCALE", 1.0)
...
def _set_arviz_ci_defaults():
    try:
        if "stats.ci_prob" in az.rcParams:
            az.rcParams["stats.ci_prob"] = HDI_PROB1
        if "stats.ci_kind" in az.rcParams:
            az.rcParams["stats.ci_kind"] = "eti"
    except Exception:
        pass


def _get_summary_eti_columns(summary: pd.DataFrame) -> tuple[str, str]:
...
def _get_prior_dataset(idata):
    ...
    try:
        ...
        raise AttributeError("prior group not found on inference data") from exc


def _calc_eti_from_sample_matrix(samples_2d: np.ndarray, prob: float = HDI_PROB1) -> np.ndarray:
...
def _calc_interval_overlap_mask(values: np.ndarray, sigma: np.ndarray, lower: np.ndarray, upper: np.ndarray, sigma_scale: float = 1.0) -> np.ndarray:
    ...
    return valid & (np.maximum(value_lower, lower) <= np.minimum(value_upper, upper))


def _get_interval_value_formatter(values: np.ndarray):
...
def _format_pair_interval_title(median: float, lower_err: float, upper_err: float, unit_label: str = "") -> str:
    median = float(median)
    ...
    if np.isfinite(median) and abs(median) >= 1000:
        ...
        title_text = rf"{median_text}_{{-{lower_text}}}^{{+{upper_text}}}\times 10^{{{exponent}}}"
    else:
        formatter = _get_interval_value_formatter(np.asarray([median, median - lower_err, median + upper_err], dtype=float))
        title_text = rf"{formatter(median)}_{{-{formatter(lower_err)}}}^{{+{formatter(upper_err)}}}"
...
def _annotate_pair_marginals(pair_axes, flat_trace, plotted_var_names: list[str], title_fontsize: float = 9, plot_median_line: bool = False) -> None:
    ...
    for idx in range(diagonal_count):
        ...
        if var_name not in flat_trace:
            continue

        samples = np.asarray(flat_trace[var_name].values, dtype=float).reshape(-1)
        ...
        title_text = _format_pair_interval_title(median, lower_err, upper_err, unit_label)
        ax.set_title(
            rf"${title_text}$",
...
def _fit_log10_mc_gmm(samples_2d: np.ndarray, max_components: int = 3, random_state: int = 42) -> dict:
    """
    Fit a Gaussian Mixture Model (GMM) in log10(M200)-log10(c) space.
    Returns serializable parameters for downstream stage-2 inference.
    Falls back to a single Gaussian if sklearn is unavailable or fitting fails.
    """
    ...
    if n_samples < 5:
        mu = np.mean(samples, axis=0) if n_samples > 0 else np.array([np.nan, np.nan], dtype=float)
        cov = np.cov(samples, rowvar=False) if n_samples > 1 else np.eye(2, dtype=float)
        return {
